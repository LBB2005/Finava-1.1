/* ============================================================
   ...
   Sourced from the public S&P 500 constituents dataset; regenerate
   when membership changes. Failure-isolated downstream, so a stale
   add/drop just means one row is missing, never a crash.
   ============================================================ */
...
export const SP500: Constituent[] = ROWS.map(([ticker, name, sector]) => ({ ticker, name, sector }));

/** All GICS sectors present in the universe, sorted. */
export const SP500_SECTORS: string[] = Array.from(new Set(SP500.map((c) => c.sector))).sort();

/** ticker → constituent lookup. */
export const SP500_BY_TICKER: Map<string, Constituent> = new Map(SP500.map((c) => [c.ticker, c]));
